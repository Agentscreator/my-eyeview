...
export default function PricingPage() {
  return (
    <div className="min-h-screen">
      ...
      <section className="bg-gray-50 py-12 sm:py-16 md:py-20 lg:py-24 px-4 sm:px-6 md:px-8">
        <div className="container max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
            {pricingPlans.map((plan, index) => (
              <div
                key={index}
                className={`relative bg-white rounded-2xl sm:rounded-3xl p-6 sm:p-8 space-y-5 sm:space-y-6 transition-all hover:shadow-2xl ${
                  plan.popular
                    ? "border-3 sm:border-4 border-[#4d86ad] shadow-xl md:scale-105"
                    : "border-2 border-gray-200 hover:border-[#4d86ad]"
                }`}
              >
                {plan.popular && (
                  <div className="absolute -top-3 sm:-top-4 left-1/2 -translate-x-1/2 bg-[#4d86ad] text-white px-4 sm:px-6 py-1.5 sm:py-2 rounded-full text-xs sm:text-sm font-bold whitespace-nowrap">
                    MOST POPULAR
                  </div>
                )}

                <div className="space-y-2 pt-2">
                  <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-black">{plan.name}</h3>
                  <p className="text-sm sm:text-base text-gray-600 leading-snug">{plan.description}</p>
                </div>
                ...
                <ul className="space-y-2.5 sm:space-y-3 min-h-[180px] sm:min-h-[200px]">
                  {plan.features.map((feature, featureIndex) => (
                    <li key={featureIndex} className="flex items-start gap-2 sm:gap-3">
                      <Check className="w-4 h-4 sm:w-5 sm:h-5 text-[#4d86ad] flex-shrink-0 mt-0.5" strokeWidth={3} />
